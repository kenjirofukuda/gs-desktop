...
- (BOOL) readFromData: (NSData *)data ofType: (NSString *)typeName error: (NSError **)outError
{
  ...
  NSDictionary *properties = [NSPropertyListSerialization propertyListFromData: data mutabilityOption:
  ...
  if (properties)
    {
      ...
      graphics = [graphicPropertiesArray isKindOfClass: [NSArray class]] ? [SKTGraphic
                                                                            graphicsWithProperties: graphicPropertiesArray] : [NSArray array];
  ...
  if (readSuccessfully)
    {
      ...
      @try
        {
          [self removeGraphicsAtIndexes: [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                            0, [[self graphics] count])]];
          [self insertGraphics: graphics atIndexes: [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                                       0, [graphics count])]];
        }
  ...
  @try
    {
      ...
      [properties setObject: [NSNumber numberWithInteger: SKTDocumentCurrentVersion] forKey:
                  SKTDocumentVersionKey];
      [properties setObject: [SKTGraphic propertiesWithGraphics: graphics] forKey:
                  SKTDocumentGraphicsKey];
      //error while archiving!
      //[properties setObject:[NSArchiver archivedDataWithRootObject:printInfo] forKey:SKTDocumentPrintInfoKey];

      data = [NSPropertyListSerialization dataFromPropertyList: properties format:
                                          NSPropertyListBinaryFormat_v1_0 errorDescription: NULL];
  ...
  (NSError **)outError
{
  ...
  SKTRenderingView *renderingView = [[SKTRenderingView alloc] initWithFrame: NSMakeRect(0.0, 0.0,
                                                                                        documentSize.width, documentSize.height) graphics: [self graphics] printJobTitle: printJobTitle];
