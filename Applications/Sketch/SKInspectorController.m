...
{
  NSArray *sel = [NSApp valueForKeyPath:
                        @"mainWindow.windowController.graphicsController.selectedObjects"];
...
- (IBAction) updateSelection: (id)sender
{
  ...
  if (sender == fillColorWell)
  ...
  else if (sender == lineButtton)
    {
      for (id it in sel)
        {
          [it setValue: [NSNumber numberWithInteger: [lineButtton state]] forKey:
  ...
  else if (sender == lineWidthField)
    {
      ...
      for (id it in sel)
        {
          [it setValue: [NSNumber numberWithInteger: [lineWidthField integerValue]] forKey:
