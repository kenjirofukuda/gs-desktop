...
static NSString *SKTGraphicViewIndividualGraphicObservationContext =
  @"com.apple.SKTGraphicView.individualGraphic";
static NSString *SKTGraphicViewSelectionIndexesObservationContext =
  @"com.apple.SKTGraphicView.selectionIndexes";
  ...
  if (self)
    {
      ...
      [self registerForDraggedTypes: [[NSArray arrayWithObjects: NSColorPboardType, NSFilenamesPboardType,
                                       nil] arrayByAddingObjectsFromArray: [NSImage imagePasteboardTypes]]];

      // Initalize the cascading of pasted graphics.
      _pasteboardChangeCount = -1;
      _pasteCascadeNumber = 0;
      _pasteCascadeDelta = NSMakePoint(SKTGraphicViewDefaultPasteCascadeDelta,
                                       SKTGraphicViewDefaultPasteCascadeDelta);
...
- (NSMutableArray *) mutableGraphics
{
  ...
  NSAssert((_graphicsContainer
            && _graphicsKeyPath), @"An SKTGraphicView's 'graphics' property is not bound to anything.");
  NSMutableArray *mutableGraphics = [_graphicsContainer mutableArrayValueForKeyPath:
                                                        _graphicsKeyPath];
...
- (NSIndexSet *) selectionIndexes
{
  ...
  NSIndexSet *selectionIndexes = [_selectionIndexesContainer valueForKeyPath:
                                                             _selectionIndexesKeyPath];
  if (!selectionIndexes)
    {
...
- (void) changeSelectionIndexes: (NSIndexSet *)indexes
{
  ...
  NSAssert((_selectionIndexesContainer
            && _selectionIndexesKeyPath),
           @"An SKTGraphicView's 'selectionIndexes' property is not bound to anything.");
...
- (void) startObservingGraphics: (NSArray *)graphics
{
  ...
  NSIndexSet *allGraphicIndexes = [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                                0, [graphics count])];
  [graphics addObserver: self toObjectsAtIndexes: allGraphicIndexes forKeyPath:
            SKTGraphicDrawingBoundsKey options: (NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld)
            context: SKTGraphicViewIndividualGraphicObservationContext];
  ...
  [graphics addObserver: self toObjectsAtIndexes: allGraphicIndexes forKeyPath:
            SKTGraphicDrawingContentsKey options: 0 context: SKTGraphicViewIndividualGraphicObservationContext];
...
- (void) stopObservingGraphics: (NSArray *)graphics
{
  ...
  NSIndexSet *allGraphicIndexes = [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                                0, [graphics count])];
  [graphics removeObserver: self fromObjectsAtIndexes: allGraphicIndexes forKeyPath:
            SKTGraphicDrawingContentsKey];
  [graphics removeObserver: self fromObjectsAtIndexes: allGraphicIndexes forKeyPath:
            SKTGraphicDrawingBoundsKey];

}


// An override of the NSObject(NSKeyValueBindingCreation) method.
- (void) bind: (NSString *)bindingName toObject: (id)observableObject withKeyPath:
  (NSString *)observableKeyPath options: (NSDictionary *)options
{
  ...
  if ([bindingName isEqualToString: SKTGraphicViewGraphicsBindingName])
    {
      ...
      NSAssert(([options count] == 0),
               @"SKTGraphicView doesn't support any options for the 'graphics' binding.");
      ...
      [_graphicsContainer addObserver: self forKeyPath: _graphicsKeyPath options:
                          (NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:
                          SKTGraphicViewGraphicsObservationContext];
  ...
  else if ([bindingName isEqualToString: SKTGraphicViewSelectionIndexesBindingName])
    {
      ...
      NSAssert(([options count] == 0),
               @"SKTGraphicView doesn't support any options for the 'selectionIndexes' binding.");
      ...
      [_selectionIndexesContainer addObserver: self forKeyPath: _selectionIndexesKeyPath options:
                                  (NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:
                                  SKTGraphicViewSelectionIndexesObservationContext];
  ...
  else
    {
      ...
      [super bind: bindingName toObject: observableObject withKeyPath: observableKeyPath options:
       options];
      ...
      if (![oldSelectionIndexes isEqual: [NSNull null]] && ![newSelectionIndexes isEqual: [NSNull null]])
        {
          ...
          for (oldSelectionIndex = [oldSelectionIndexes firstIndex]; oldSelectionIndex != NSNotFound;
          ...
          for (newSelectionIndex = [newSelectionIndexes firstIndex]; newSelectionIndex != NSNotFound;
      ...
      if (NSIntersectsRect(rect, graphicDrawingBounds))
        {
          ...
          [graphic drawContentsInView: self isBeingCreateOrEdited: (graphic == _creatingGraphic
...
- (void) continueEchoingMoveToRulers: (NSRect)echoRect
{
  ...
  [horizontalRuler moveRulerlineFromLocation: NSMinX(oldHorizontalRect) toLocation: NSMinX(
                     newHorizontalRect)];
  [horizontalRuler moveRulerlineFromLocation: NSMidX(oldHorizontalRect) toLocation: NSMidX(
                     newHorizontalRect)];
  [horizontalRuler moveRulerlineFromLocation: NSMaxX(oldHorizontalRect) toLocation: NSMaxX(
                     newHorizontalRect)];

  [verticalRuler moveRulerlineFromLocation: NSMinY(oldVerticalRect) toLocation: NSMinY(
                   newVerticalRect)];
  [verticalRuler moveRulerlineFromLocation: NSMidY(oldVerticalRect) toLocation: NSMidY(
                   newVerticalRect)];
  [verticalRuler moveRulerlineFromLocation: NSMaxY(oldVerticalRect) toLocation: NSMaxY(
                   newVerticalRect)];
...
- (void) setNeedsDisplayForEditingViewFrameChangeNotification: (NSNotification *)
...
- (void) startEditingGraphic: (SKTGraphic *)graphic
{
  ...
  NSAssert((!_editingGraphic
            && !_editingView), @"-[SKTGraphicView startEditingGraphic:] is being mis-invoked.");
  ...
  if (_editingView)
    {
      ...
      _oldReservedThicknessForRulerAccessoryView = [[[self enclosingScrollView] horizontalRulerView]
      ...
      [[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(
                                              setNeedsDisplayForEditingViewFrameChangeNotification:) name: NSViewFrameDidChangeNotification
                                            object: _editingView];
  ...
  if (_editingView)
    {
      ...
      [horizontalRulerView setReservedThicknessForAccessoryView:
                           _oldReservedThicknessForRulerAccessoryView];
...
- (SKTGraphic *) graphicUnderPoint: (NSPoint)point index: (NSUInteger *)outIndex isSelected:
  (BOOL *)outIsSelected handle: (NSInteger *)outHandle
{
...
- (void) moveSelectedGraphicsWithEvent: (NSEvent *)event
{
  ...
  NSPoint selOriginOffset = NSMakePoint((lastPoint.x - selBounds.origin.x),
  ...
  while ([event type] != NSLeftMouseUp)
    {
      ...
      if (!isMoving && ((fabs(curPoint.x - lastPoint.x) >= 2.0)
      ...
      if (isMoving)
        {
          ...
          if (!NSEqualPoints(lastPoint, curPoint))
            {
              [[SKTGraphic class] translateGraphics: selGraphics byX: (curPoint.x - lastPoint.x) y:
                                  (curPoint.y - lastPoint.y)];
              didMove = YES;
              [self setNeedsDisplay: YES]; //XXX
              if (echoToRulers)
                {
                  [self continueEchoingMoveToRulers: NSMakeRect(curPoint.x - selOriginOffset.x,
                                                                curPoint.y - selOriginOffset.y, NSWidth(selBounds), NSHeight(selBounds))];
                }
...
- (void) resizeGraphic: (SKTGraphic *)graphic usingHandle: (NSInteger)handle withEvent:
  (NSEvent *)event
{
...
- (void) createGraphicOfClass: (Class)graphicClass withEvent: (NSEvent *)event
{
  ...
  [self resizeGraphic: _creatingGraphic usingHandle: [graphicClass creationSizingHandle] withEvent:
        event];
  ...
  while ([event type] != NSLeftMouseUp)
    {
      ...
      NSRect newMarqueeSelectionBounds = NSMakeRect(fmin(originalMouseLocation.x, currentMouseLocation.x),
      ...
      if (!NSEqualRects(newMarqueeSelectionBounds, _marqueeSelectionBounds))
        {
          ...
          NSIndexSet *indexesOfGraphicsInRubberBand = [self indexesOfGraphicsIntersectingRect:
                                                            _marqueeSelectionBounds];
          NSMutableIndexSet *newSelectionIndexes = [oldSelectionIndexes mutableCopy];
          NSUInteger index;
          for (index = [indexesOfGraphicsInRubberBand firstIndex]; index != NSNotFound;
...
- (void) selectAndTrackMouseWithEvent: (NSEvent *)event
{
  ...
  SKTGraphic *clickedGraphic = [self graphicUnderPoint: mouseLocation index: &clickedGraphicIndex
                                     isSelected: &clickedGraphicIsSelected handle: &clickedGraphicHandle];
  if (clickedGraphic)
    {
...
- (void) mouseDown: (NSEvent *)event
{
  ...
  Class graphicClassToInstantiate = [[SKTToolPaletteController sharedToolPaletteController]
                                     currentGraphicClass];
  if (graphicClassToInstantiate)
  ...
  else
    {
      ...
      if ([event clickCount] > 1)
        {
          NSPoint mouseLocation = [self convertPoint: [event locationInWindow] fromView: nil];
          doubleClickedGraphic = [self graphicUnderPoint: mouseLocation index: NULL isSelected: NULL handle:
                                       NULL];
...
{
  [_handleShowingTimer invalidate];
  _handleShowingTimer = [NSTimer scheduledTimerWithTimeInterval: 0.5 target: self selector: @selector(
                                   unhideHandlesForTimer:) userInfo: nil repeats: NO];
  _isHidingHandles = YES;
  [self setNeedsDisplayInRect: [SKTGraphic drawingBoundsOfGraphics: [self selectedGraphics]]];
}
...
- (IBAction) copy: (id)sender
{
  ...
  [pasteboard declareTypes: [NSArray arrayWithObjects: SKTGraphicViewPasteboardType, NSPDFPboardType,
                             NSTIFFPboardType, nil] owner: nil];
  [pasteboard setData: [[SKTGraphic class] pasteboardDataWithGraphics: selectedGraphics] forType:
              SKTGraphicViewPasteboardType];
  [pasteboard setData: [[SKTRenderingView class] pdfDataWithGraphics: selectedGraphics] forType:
              NSPDFPboardType];
  [pasteboard setData: [[SKTRenderingView class] tiffDataWithGraphics: selectedGraphics error: NULL]
...
- (IBAction) paste: (id)sender
{
  ...
  NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
  NSString *typeName = [pasteboard availableTypeFromArray: [NSArray arrayWithObjects:
                                                            SKTGraphicViewPasteboardType, NSFilenamesPboardType, nil]];
  if ([typeName isEqualToString: SKTGraphicViewPasteboardType])
    {
      ...
      NSError *error;
      NSArray *graphics = [graphicClass graphicsWithPasteboardData: [pasteboard dataForType: typeName]
                                        error: &error];
      if (graphics)
        {
          ...
          if (graphicCount > 0)
            {
              ...
              if (_pasteCascadeNumber > 0)
                {
                  [graphicClass translateGraphics: graphics byX: (_pasteCascadeNumber * _pasteCascadeDelta.x) y:
                                (_pasteCascadeNumber * _pasteCascadeDelta.y)];
                }
              _pasteCascadeNumber++;

              // Add the pasted graphics in front of all others and select them.
              NSIndexSet *insertionIndexes = [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(0,
                                                                                                 graphicCount)];
      ...
      else
        {
          ...
          [self presentError: error modalForWindow: [self window] delegate: nil didPresentSelector: NULL
                contextInfo: NULL];
      ...
      if ([filenames count] == 1)
        {
          ...
          [self makeNewImageFromContentsOfFile: filename atPoint: NSMakePoint(50, 50)];
        }
    }
  else
    {
      [self makeNewImageFromPasteboard: pasteboard atPoint: NSMakePoint(50, 50)];
    }
...
- (BOOL) validateMenuItem: (NSMenuItem *)item
{
  ...
  if (action == @selector(makeNaturalSize:))
  ...
  else if ((action == @selector(alignWithGrid:)) || (action == @selector(removeObject:))
           || (action == @selector(bringToFront:)) || (action == @selector(sendToBack:))
  ...
  else if ((action == @selector(alignLeftEdges:)) || (action == @selector(alignRightEdges:))
           || (action == @selector(alignTopEdges:)) || (action == @selector(alignBottomEdges:))
           || (action == @selector(alignHorizontalCenters:)) || (action == @selector(alignVerticalCenters:))
           || (action == @selector(makeSameWidth:)) || (action == @selector(makeSameHeight:)))
    {
  ...
  else if (action == @selector(showOrHideRulers:))
    {
      ...
      [item setTitle: ([[self enclosingScrollView] rulersVisible] ? NSLocalizedStringFromTable(
                         @"Hide Ruler", @"SKTGraphicView",
                         @"A main menu item title.") : NSLocalizedStringFromTable(@"Show Ruler", @"SKTGraphicView",
  ...
  if ([selectionIndexes count] > 0)
    {
      ...
      NSIndexSet *insertionIndexes = [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                                   0, [selectedObjects count])];
  ...
  if ([selectionIndexes count] > 0)
    {
      NSMutableArray *mutableGraphics = [self mutableGraphics];
      [mutableGraphics removeObjectsAtIndexes: selectionIndexes];
      NSIndexSet *insertionIndexes = [NSIndexSet indexSetWithIndexesInRange: NSMakeRange([mutableGraphics
...
- (void) changeColor: (id)sender
{
  ...
  if ([inspector isSelectingLineColor])
    {
      [[self selectedGraphics] makeObjectsPerformSelector: @selector(setStrokeColor:) withObject: [sender
                                                                                                   color]];
    }
  else
    {
      [[self selectedGraphics] makeObjectsPerformSelector: @selector(setFillColor:) withObject: [sender
                                                                                                 color]];
    }
}
...
{
  [self changeSelectionIndexes: [NSIndexSet indexSetWithIndexesInRange: NSMakeRange(
                                   0, [[self graphics] count])]];
}
